...
# Function to normalize the labels using percentile-based scaling
...
# The function takes in the labels and a percentile value p, and returns the normalized labels
# and the range used for normalization.
# The normalization is done by subtracting the p-th percentile from the labels and dividing
# by the difference between the (1-p)-th and p-th percentiles.
# The function also returns the range used for normalization, which includes the p-th and (1-p)-th percentiles.
# The function is designed to work with numpy arrays and can handle multi-dimensional data.
# percentile scaling focuses on the central bulk of the data and ignores extreme values
...
# Inverse the normalization process by scaling the normalized labels back to the original range
...
def train_model(model, train_loader, val_loader, loss_function, learning_rate, num_epochs, patience,
    ...
    for epoch in range(num_epochs):
        ...
        for step, (batch_spectra, batch_labels) in enumerate(train_loader): #Each "step" is one batch
